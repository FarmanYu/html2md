...
htmlParser.prototype = {
	handle:function(html){
		...
		this.els.each(function(idx, el){
			self.switchcase(el.name, el);
		});
		return this._markdown;
	},
	switchcase:function( tagneme, el ){
		var self = this;
		switch(tagneme){
				 case 'p':
				  self.handle_p(el);
	...
	handle_a:function( el ){
		var title = $(el).attr("title") == "" ? "" : '"'+ $(el).attr("title") +'"';
		var output = '['+ $(el).text() +']('+ $(el).attr("href") +' '+ title +')';
		this.concat( output );
	},
	handle_img:function(el){
		var src = $(el).attr('data-original') || $(el).attr("data-src") || $(el).attr("src");
		this.concat('['+ $(el).attr("alt") +']('+ src +')');
	},
	handle_p:function( el ){
		var self = this;
		if(el.children.length > 10){
			var childrens = $(el).children();
			childrens.each(function(idx, ele){
				 self.switchcase(ele.name, ele);
			});
		} else{
			this.concat( $(el).text() +"  \n" );
		}
